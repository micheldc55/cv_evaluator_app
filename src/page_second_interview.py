...
from src.file_management.file_management import (
    ...
    load_metadata,
    load_second_interview_questions,
    save_metadata,
    save_second_interview_qestions,
)
from src.streamlit.streamlit_utils import add_n_whitespaces, display_pdf, write_status_in_colors


def second_interview_page():
    st.title("Interview")
    candidate_list = get_all_candidates_for_second_interview()
    ...
    if not candidate_list:
        ...
        write_status_in_colors(metadata.status_second_interview.upper(), STATUS_COLOR_MAPPING)
        add_n_whitespaces(2)
        ...
        with col2:
            ...
            iq_list = load_second_interview_questions(selected_candidate)
            ...
            if st.button("Add Question"):
                if new_question.strip() != "":
                    ...
                    save_second_interview_qestions(selected_candidate, iq_list)
            ...
            with col1:
                if st.button("Approve Candidate"):
                    metadata.status_second_interview = APPROVED
            ...
            with col2:
                if st.button("Reject Candidate"):
                    metadata.status_second_interview = REJECTED
                    save_metadata(selected_candidate, metadata)
